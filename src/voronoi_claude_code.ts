...
class Voronoi {
  ...
  RBTree = class {
    ...
    rbInsertSuccessor(node: any, successor: any): void {
      let parent;
      if (node) {
  ...
  Cell = class {
    ...
    prepareHalfedges(): number {
      ...
      halfedges.sort((a: any, b: any) => b.angle - a.angle);
  ...
  compute(sites: Site[], bbox: BBox): any {
    ...
    let site = siteEvents.pop(),
      siteid = 0,
      xsitex: number | undefined, // to avoid duplicate sites
